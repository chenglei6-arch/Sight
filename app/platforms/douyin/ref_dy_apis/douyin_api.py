...
本文件基本照搬自 cv-cat/DouYin_Spider 项目:
  https://github.com/cv-cat/DouYin_Spider

原项目协议: Apache-2.0 License

主要修改:
  - 将 has_more != 1 改为 not res_json.get("has_more") 以兼容布尔返回值
  - 移除了 protobuf 解析依赖中的部分非必要处理
  - 增加了翻页数量参数的灵活控制
"""
import json
import random
...
from app.platforms.douyin.ref_utils.dy_util import splice_url, generate_a_bogus, generate_msToken, trans_cookies
...
class DouyinAPI:
    ...
    def search_some_general_work(auth, query: str, num: int, sort_type: str, publish_time: str, filter_duration="", search_range="", content_type="", **kwargs) -> list:
        ...
        while True:
            ...
            works = res_json["data"]
            work_list.extend(works)
            if not res_json.get("has_more") or len(work_list) >= num:
                break
    ...
    def search_some_live(auth, query: str, num: int, **kwargs) -> list:
        ...
        while True:
            ...
            if not res_json.get("has_more") or len(live_list) >= num:
                break
            offset = str(int(offset) + int(count))
    ...
    def get_some_user_follower_list(auth, user_id: str, sec_id: str, num: int, **kwargs) -> list:
        ...
        while True:
            ...
            if not res_json.get("has_more") or len(follower_list) >= num:
                break
    ...
    def get_some_user_following_list(auth, user_id: str, sec_id: str, num: int, **kwargs) -> list:
        ...
        while True:
            ...
            if not res_json.get("has_more") or len(following_list) >= num:
                break
            max_time = res_json["min_time"]
    ...
    def get_some_notice_list(auth, num: int = 20, notice_group='700', **kwargs) -> list:
        ...
        while True:
            ...
            if not res_json.get("has_more") or len(notice_list) >= num:
                break
    ...
    def search_some_video_work(auth, query: str, num: int = 16, sort_type: str = '0', publish_time: str = '0',
        ...
        while True:
            ...
            if not res_json.get("has_more") or len(video_work_list) >= num:
                break
            offset = str(int(offset) + int(count))
