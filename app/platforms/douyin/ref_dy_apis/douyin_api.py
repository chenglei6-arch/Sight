...
from app.platforms.douyin.ref_utils import http_client as requests
...
import app.platforms.douyin.ref_static.Response_pb2 as ResponseProto
from app.platforms.douyin.ref_builder.header import HeaderBuilder, HeaderType
from app.platforms.douyin.ref_builder.params import Params
from app.platforms.douyin.ref_builder.proto import ProtoBuilder
from app.platforms.douyin.ref_utils.fingerprint import get_profile
from app.platforms.douyin.ref_utils.dy_util import splice_url, generate_a_bogus, generate_msToken, trans_cookies, generate_a_bogus_pure
...
class DouyinAPI:
    ...
    def get_user_info(auth, user_url: str, **kwargs) -> dict:
        ...
        # 本项目适配：数字 UID 用 user_id 查询，sec_uid 用 sec_user_id
        # （上游只留了 sec_user_id，但监控目标的 uid 多为数字形态）
        if user_id.isdigit():
    ...
    def get_user_following_list(auth, user_id: str, sec_id: str, max_time: str = '0', count: str = '20', **kwargs):
        ...
        # 与 follower/list 同款实录结论：max_time 传当前秒级时间戳（source_type=1）
        # 才返回列表；传 0 时服务端走另一条分支，只回 mix_count 不回 followings
        if not max_time or max_time == '0':
            max_time = str(int(time.time()))
        headers = HeaderBuilder().build(HeaderType.GET)
        refer = f"https://www.douyin.com/user/{sec_id}"
    ...
    def send_image(auth, conversation_id, conversation_short_id, ticket,
        ...
        from app.platforms.douyin.ref_dy_apis.douyin_im_media import DouyinIMMedia
        upload_kwargs = {
