...
from app.platforms.douyin.ref_utils.dy_util import (trans_cookies, generate_msToken, generate_dynamic_msToken,
                           generate_s_v_web_id)
...
class DouyinAuth:
    ...
    def from_cookie(cls, cookie_str, *, ticket=None, ts_sign=None,
        ...
        if auth.cookie.get("__ac_nonce") and not auth.cookie.get("__ac_signature"):
            try:
                from app.platforms.douyin.ref_dy_apis.login_api import DYLoginApi
    ...
    def ensure_http_session(self):
        ...
        if self.http is None:
            from app.platforms.douyin.ref_utils.http_client import Session
            self.http = Session()
    ...
    def request(self, method, url, **kwargs):
        ...
        try:
            from app.platforms.douyin.ref_utils.passport import merge_set_cookies
    ...
    def capability_report(self):
        ...
        """
        try:
            from app.platforms.douyin.ref_dy_apis.login_api import anonymous_cookie_evidence
    ...
    def bootstrap_creator_session(self, proxies=None):
        ...
        if self.creator_bootstrapped:
            return self
        from app.platforms.douyin.ref_builder.header import HeaderBuilder
        from app.platforms.douyin.ref_utils.dy_util import generate_s_v_web_id
    ...
    def ecdh_key(self, aid=6383, origin="https://www.douyin.com"):
        ...
        key = None
        try:
            from app.platforms.douyin.ref_utils.bd_ticket import derive_ecdh_key, fetch_server_cert
            from app.platforms.douyin.ref_builder.header import HeaderBuilder
    ...
    def webid(self):
        ...
        if self._webid_resolving:
            from app.platforms.douyin.ref_utils.dy_util import generate_fake_webid
        ...
        try:
            ...
            self._webid_resolving = False
        if not wid:
            from app.platforms.douyin.ref_utils.dy_util import generate_webid
    ...
    def session_dtrait_header(self, path, aid=6383, origin="https://www.douyin.com",
        ...
        if not blob and self.dtrait_profile:
            try:
                from app.platforms.douyin.ref_utils.dtrait_features import build_blob
        ...
        if not blob:
            ...
            return None
        try:
            from app.platforms.douyin.ref_utils.dtrait import (
    ...
    def refresh_mstoken(self, common=False, common_behavior=False, sms=False):
        ...
        if common:
            ...
            try:
                from app.platforms.douyin.ref_utils.mstoken import refresh_common_mstoken
